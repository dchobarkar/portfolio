...
function calculateLinesToHighlight(data) {
  if (rExpression.test(data)) {
    const lineNumbers = rExpression
      .exec(data)[1]
      .split(',')
  ...
  return (
    <Highlight {...defaultProps} code={codeString} language={language}>
      {({ className, tokens, getLineProps, getTokenProps }) => {
        ...
        const preClass = `${className} group relative p-4${
          multiline.length ? ' pt-3 pb-5 my-0' : ''
        }`;
        return (
